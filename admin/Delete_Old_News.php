	...
	<!-- middle content starts here where vertical nav slides and news ticker statr -->
			...
			<div  class="col-sm-1 col-md-1 col-lg-1"  >
				...
				<div  class="col-sm-10 col-md-10 col-lg-10">
					<div  class="col-lg-12" style="width:100%; padding-top:5px; padding-left:5px; padding-bottom:10px; background-color:grey;margin-bottom:1%">
						<h3 style="text-align:center;color:white">The Profiler - Delete News</h3>
					...
					<div  class="col-sm-12 col-md-12 col-lg-12"  class="col-lg-12" style="width:100%;  padding:10px; background-color:CadetBlue;margin-bottom:1%;">
							<?php
											...
											$status="";
											$sql = "(SELECT * FROM news where status='".$status."' ORDER BY id DESC LIMIT {$per_page} OFFSET {$offset})"; 
											$retval = mysql_query($sql, $conn_two);
											if (!$retval) {
												die('Could not get data: ' . mysql_error());
											}else{
											...
											$dateprint = "For : ".$J.", ".$Q;	
											echo '<h4 style="text-align:left;color:yellow">List Of All News - '.$dateprint.'</h4><p id="result2"></p>';
											...
											/*	$formattedNum_one =  number_format($row_retrieve['price'], 2);
												$Amount_Naira =" &#8358; ".$formattedNum_one;
												 */
												 $subj = substr($row_retrieve['news_info'],0,100);
												 $body = $subj."...";
												 $news_info = htmlspecialchars_decode($body);
												$data=$data.'<tr >
																	<td>'.$row_retrieve['id'].'</td>
																	<td>'.$row_retrieve['news_head'].'</td>
																	<td>'.$row_retrieve['news_id'].'</td>
																	<td>'.$news_info.'</td>
																	<td>'.$row_retrieve['news_date'].'</td>
																	<td><a href="#" title="Delete_Item" style="color:yellow" onclick="delete_Cart_Two(\''.$row_retrieve['news_id'].'\',\''.$row_retrieve['news_head'].'\')"><img src="../store_files/images/delete-icon.jpg" style="height:20px" ></img></a></td>
															</tr>';
													$j =$j +1;
											}
											echo $data.'</table>';
											...
											if ($total_pages > 1)
											{
												...
												{
												echo ' <li><a href=Delete_Old_News.php?page='.$previous_page.'>&laquo; </a> </li>';
												}
												 ...
												 for($i = 1; $i <= $total_pages; $i++)
												 {
													...
													//then it sset the i to be active or focused
													 ...
													 //display the page number
														echo ' <li><a href=Delete_Old_News.php?page='.$i.'> '. $i .' </a></li>';
													 }
												 }
												//this is for next record		
												if ($has_next_page)
												{
													echo ' <li><a href=Delete_Old_News.php?page='.$next_page.'>&raquo;</a></li> ';
												}
		...
		<div class="row" >
			<div class="col-xs-1 col-sm-1"></div>	
				<div class="col-xs-10 col-sm-10" style="background-color:grey;" >
					<footer>
