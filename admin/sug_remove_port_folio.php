		...
		<div class="row">
				...
				<div  class="col-sm-11 col-md-11 col-lg-11">
					<div  class="col-lg-12" style="width:100%; padding-top:5px; padding-left:5px; padding-bottom:10px; background-color:grey;margin-bottom:1%">
						<h3 style="text-align:center;color:white">A T B U Election | Remove Portfolio</h3>
					...
					<div  class="col-sm-12 col-md-12 col-lg-12"  class="col-lg-12" style="width:100%;  padding:10px; background-color:CadetBlue;margin-bottom:1%;">
							<?php
											...
											$sql    = "SELECT count(id) FROM sug_post_list";
											...
											$status="0";
											$sql = "(SELECT * FROM sug_post_list where del_status='".$status."' ORDER BY sug_post ASC LIMIT {$per_page} OFFSET {$offset})"; 
											$retval = mysql_query($sql, $conn_two);
											if (!$retval) {
												die('Could not get data: ' . mysql_error());
											}else{
											...
											$dateprint = "For : ".$J.", ".$Q;	
											echo '<h4 style="text-align:left;color:yellow">List Of All Portfolio - '.$dateprint.'</h4><p id="result2"></p>';
											echo '<table border="1" class="table table-condensed">
																		<thead style="background-color:grey;color:yellow">
																			<tr>
																				<th>S/N<u>o</u>.</th>
																						<th>Title of Portfolio</th>
																						<th>Portfolio Id</th>
																						<th></th>
																			</tr>
																		</thead>
											...
											while ($row_retrieve = mysql_fetch_array($retval, MYSQL_ASSOC)) 
											{
												...
												$data=$data.'<tr >
																	<td>'.$j.'</td>
																	<td>'.$row_retrieve['sug_post'].'</td>
																	<td>'.$row_retrieve['sug_post_code'].'</td>
																	
																	<td><a href="#" title="Delete_Item" style="color:yellow" 
																	onclick="delete_Cart_Two(\''.$row_retrieve['sug_post_code'].'\',\''.$row_retrieve['sug_post'].'\')">
