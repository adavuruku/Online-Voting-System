...
if(!isset($_SESSION['reg_oo']) or !isset($_SESSION['vote_details']))
{
	header("location: sign_logout.php");
				...
				$stud_info="";
				$d="1";
				$stmt = $conn->prepare("SELECT reg_No,v_code_status,pics_ext,Stud_Name,date_vote_code,vote_details FROM student_information WHERE reg_No=? and v_code_status=? Limit 1");
				$stmt->execute(array($_voter_regno,$d));
				$affected_rows = $stmt->rowCount();
				if($affected_rows >= 1) 
				{
					$row = $stmt->fetch(PDO::FETCH_ASSOC);
					$_SESSION['all_vote_id'] = $row['vote_details'];
					$set_time = $row['date_vote_code'];
					...
					if($hours>=1){
						...
						$stmt->execute(array($stat,$row['reg_No']));
						$affected_rows = $stmt->rowCount();
					$notice_msg_u = "<p style='color:blue;font-weight:bold;'> Error: You Dont Have A Valid Pin Your Pin has Expired ! request for another pin to continue voting<p>";
								...
								if($affected_rows >= 1) 
								{
									...
									while($row = $stmt->fetch(PDO::FETCH_ASSOC))
									{
										...
										if($affected_rows2 >= 1) 
										{
											...
											$link_id_vote ='id="'.$row['sug_post_code'].'A"';
													...
													<div class="table-responsive">
														<table class="table table-hover table-condensed">
														   <tbody>
															  ...
															  {
																$verify_all= explode(';',$_SESSION['all_vote_id']);
																$arrlength= count($verify_all);
																$new_list = array();
																for($x=0; $x < $arrlength; $x++)
																{
																  $oyiza=explode('-',$verify_all[$x]);
																  array_push($new_list,$oyiza[1]);
																}
																$coler = $row['sug_post_code'];
																  if(!in_array($coler,$new_list))
																  {
																	$data =$data.'<tr rowspan="2">
																	 <td '.$link_id.' colspan="2" style="background-color:yellow;Color:red;font-weight:bold;
																	 border-width:5px solid;cursor:pointer;padding:10px;width:auto;"><span   
																	 onclick="Add_to_Cart(\''.$row2['c_vote_id'].'\',\''.$row['sug_post_code'].'\')" > | - Click here to Vote - | </span></td>
																	</tr>';
																  }
										...
										$display_all =$display_all.'
												...
												<input type="hidden" '.$link_id_hide.$value_link_hide.' ></input>
