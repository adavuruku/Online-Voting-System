...
if(!isset($_GET['searchmail']))
{
	...
	$hide=strip_tags($_GET['searchmail']);
}
if($hide==""){
	header("location: View_Acredited_Voters.php");
}
	...
	<div class="row" >
        <div class="col-xs-12 col-sm-12">
			<div class="col-xs-12 col-sm-12" style="background-color:grey;font-weight:bold;Color:white;padding-top:15px;">
					<form role="form" name="mailform" class="form-horizontal" action="Search_Voters_Result.php" enctype="multipart/form-data" method="GET">
						<div class="form-group">
							<label for="inputEmail" class="control-label col-xs-3">Enter Reg_No / Any Part Of your Name :</label>
							<div class="col-xs-7">
								<input type="text" class="form-control" id="inputEmail" name="searchmail" value="<?php echo $_GET['searchmail']; ?>" placeholder="Enter Reg_No / Any Part of your Name to Search">
							</div>
							<div class=" col-lg-2">
			...
			<div class="col-xs-12 col-sm-12" style="background-color:white;font-weight:bold;font Colour:red">
				<h4 style="color:blue;text-align:center;">Result of Search On Acredited Voters</h4>
				<?php
					...
					$status="";$_se=$_GET['searchmail'];
					//$search_2="Stud_Name like '%".$_se."%' OR reg_No like '%".$_se."%'";
					
					$sql = "SELECT count(*) FROM student_information where (Stud_Name like '%".$_se."%' OR reg_No like '%".$_se."%') AND (phone!='".$status."' AND Email!='".$status."')";
					...
					$status="";
					$sql = "(SELECT * FROM student_information where (Stud_Name like '%".$_se."%' OR reg_No like '%".$_se."%') AND (phone!='".$status."' AND Email!='".$status."') ORDER BY id ASC LIMIT {$per_page} OFFSET {$offset})"; 
					$retval = mysql_query($sql, $conn);
					if (!$retval) {
						die('Could not get data: ' . mysql_error());
					}else{
